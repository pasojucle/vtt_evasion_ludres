...
class IdentitySubscriber implements EventSubscriberInterface
{
    ...
    public function preSetData(FormEvent $event): void
    {
        ...
        $kinship = Identity::TYPE_MEMBER < $type;
        ...
        if ($options['is_kinship'] === $kinship) {
            $form
                ...
                ->add('isKinship', HiddenType::class, [
                    'data' => $options['is_kinship'],
                    'mapped' => false,
                ])
            ...
            if ($kinship) {
                ...
                $form
                    ->add('otherAddress', CheckboxType::class, [
                        ...
                        'mapped' => false,
                        'attr' => [
                            'class' => 'identity-other-address form-modifier',
                            'data-modifier' => sprintf('address-%s', $form->getName())
                        ],
        ...
        $this->modifier($event->getForm(), $type, $options['is_kinship'], $identity->hasAddress());
    }

    public function preSubmit(FormEvent $event): void
    {
        $data = $event->getData();
        $type = ($data && array_key_exists('type', $data)) ? (int) $data['type'] : Identity::TYPE_MEMBER;
        $kinship = Identity::TYPE_MEMBER < $type;
        $isKinship = ($data && array_key_exists('isKinship', $data)) ? (int) $data['isKinship'] : false;

        if ($isKinship === $kinship) {
            $hasOtherAddress = ($data && array_key_exists('otherAddress', $data)) ? (bool) $data['otherAddress'] : false;
            if (!$hasOtherAddress) {
                $data['address'] = null;
                $event->setData($data);
            }
            $this->modifier($event->getForm(), $type, $isKinship, $hasOtherAddress);
        }
    }

    private function modifier(FormInterface $form, int $type, bool $isKinship, bool $hasOtherAddress): void
    {
        $kinship = Identity::TYPE_MEMBER < $type;
        if ($isKinship === $kinship) {
            $addressClass = (Identity::TYPE_MEMBER !== $type) ? ' identity-address' : '';
            $addressRequired = 'required';
            if (Identity::TYPE_MEMBER !== $type && !$hasOtherAddress) {
                $addressRequired = '';
            }
            $form
