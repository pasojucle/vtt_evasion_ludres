...
class SlideShowVoter extends Voter
{
    ...
    protected function voteOnAttribute(string $attribute, mixed $subject, TokenInterface $token, ?Vote $vote = null): bool
    {
        ...
        $isUserWithPermission = $isActiveUser && $member->hasPermissions(PermissionEnum::SLIDESHOW);

        return match ($attribute) {
            self::EDIT => $this->canEdit($token, $member, $subject, $isActiveUser, $isUserWithPermission),
            self::VIEW, self::ADD => $this->canView($token, $member, $subject, $isActiveUser, $isUserWithPermission),
    ...
    private function canEdit(TokenInterface $token, Member $member, SlideshowDirectory|SlideshowImage|null $subject, bool $isActiveUser, bool $isUserWithPermission): bool
    {
        return $this->accessDecisionManager->decide($token, ['ROLE_ADMIN']) || $isUserWithPermission;
    }

    private function canView(TokenInterface $token, Member $member, SlideshowDirectory|SlideshowImage|null $subject, bool $isActiveUser, bool $isUserWithPermission): bool
    {
