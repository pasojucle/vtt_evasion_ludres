...
export default function ChapterDelete({chapter, loadSection}: ArticleDeleteProps): React.JSX.Element {
    ...
    const confirmDeleteSection = async() => {
        ...
        dataSender('DELETE', 'chapters', chapter?.id, token).then((response) => {
            if (204 === response.status) {
                toast.success(`Suppression ${chapter?.title} réussi.`);
                loadSection();
            }
        });
        setOpen(false)
            ...
            <div className={overlayClassName()} tabIndex={-1} role="dialog">           
                <div className={ dialogClassName() } role="document">
                    ...
                    <div className="p-4 md:p-5">
                        <p>Attention : la suppression d'un chapitre supprimera tous les arcles liés. Cette suppression est définitive.</p>
                        <p>{`Est-vous sûr de voiloir supprimer le chapitre ${chapter?.title} ?`}</p>
