...
export default function Chapter(): React.JSX.Element {
    const { token } = useAuth();
    const { id } = useParams();
    console.log("📦 useDataLoader appelé dans Chapter");
    ...
    const deleteArticle = (article: ArticleType) => {
        dataSender('DELETE', 'articles', article.id, token).then((response) => {
            if (204 === response.status) {
                navigate(location);
                toast(`Suppression ${article.title} réussi.`)
            }
        });
    }

    const ButtonGroup = ({article}:{article: ArticleType}) => {
        if (token) {
            return (
                <div className='ml-auto inline-flex rounded-md shadow-xs' role='group'>
                    <button type="button" onClick={() => {show('Modifier', 'article', 'lg', 'articles', article.id)}}
                ...
                <div className='max-w-3xl mx-auto xl:max-w-none xl:mr-[17.5rem] xl:pr-16'>
                    <div className="relative z-20 prose prose-slate mt-8 dark:prose-dark">
                        { data.articles.map((article: ArticleType) =>
                            <div key={article.id} id={String(article.id)} className="max-w rounded overflow-hidden shadow-lg bg-gray-100 dark:bg-gray-800 mb-5">
